...
export const ARPreview: React.FC<ARPreviewProps> = ({ product, onClose }) => {
  ...
  return (
    <div className="ar-preview-container relative w-full h-[500px] bg-neutral-100 rounded-lg overflow-hidden">
      ...
      {/* AR操作コントロール - ARモード中はAR空間内にも表示される */}
      {isModelViewerLoaded && (
        <div id="ar-controls" className="absolute bottom-4 left-4 flex flex-col gap-2 z-10">
          <button 
            id="scale-up"
          ...
          <button 
            id="toggle-placement"
            className="p-2 bg-white text-blue-600 rounded-full shadow-md mt-2 hover:bg-blue-50"
            aria-label="配置モード切替"
      ...
      {isModelViewerLoaded && showInstructions && !isARMode && (
        <div className="absolute top-4 left-0 right-0 mx-auto max-w-md bg-white p-4 rounded-lg shadow-lg text-sm">
          ...
          <ol className="list-decimal pl-5 space-y-2">
            ...
            <li>左側のボタンで拡大・縮小や配置モードを切り替えられます</li>
          </ol>
          <button 
